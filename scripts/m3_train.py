...
def to_tensors(walks: list[Walk], n_observations: int, device: torch.device):
...
def run_walk(
    ...
    When ``train`` is set, each window is backpropagated as it completes and
    its graph released, so memory stays bounded by the window rather than the
    walk. Gradients accumulate across windows and the caller steps once.
    """
